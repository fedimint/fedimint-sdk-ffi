...
# Security model: this job runs on an ephemeral GitHub-hosted runner for
# process containment, and relies on Codex's shell environment policy to
# keep provider credentials out of agent-spawned commands.
...
You are fedimint-bot, an automation agent for the fedimint-sdk-ffi GitHub
repository. fedimint-sdk-ffi provides FFI bindings for the Fedimint client
SDK: the `fedimint-client-uniffi` Rust crate wraps `fedimint-client` with
UniFFI so it can be consumed from Kotlin (Android) and Swift (iOS). An
organization member invoked you by mentioning @fedimint-bot in a GitHub
...
- The Rust crate lives in `fedimint-client-uniffi/`; run cargo commands from
  that directory.
- Avoid `unwrap()` in non-test code — exported UniFFI functions must never
  panic across the FFI boundary; return a `Result` with the exported error
  type, or use `expect()` only with an invariant message explaining why it
  cannot fail.
- Changes to exported functions, records, enums, or error variants change the
  generated Kotlin/Swift API — call out the downstream impact explicitly.
- Format with `cargo fmt --all -- --config format_code_in_doc_comments=true`
  after code changes.
- Lint with `cargo clippy --all-targets --features "uniffi/bindgen-tests"`.
- Test with
  `CLASSPATH=./tests/jna/jna-5.14.0.jar cargo test --features uniffi/bindgen-tests`
  when practical, and report what was or was not verified.
...
cd "${GITHUB_WORKSPACE}"
# Deliberately bypass Codex's own sandbox here: the ephemeral GitHub-hosted
# runner provides isolation, and shell_environment_policy keeps model/API
